...
# ADCの選定1
- 使用するFPGAはADCが搭載されていないため、外付けする必要あり
- FFTの最大周波数が6kHz→サンプリング周波数は12kHz
- 分解能は8bit程度あれば十分
  - シミュレーションにて確認済み
- 予算が限られているため、なるべく安いものを使いたい
---

# ADCの選定2
- 値段が安かったので、MCP3002を選定した。
  - 秋月電子で240円
- 3.3Vで動かした場合75ksps、5Vで動かした場合200ksps
  - サンプリングレートは低いが制度の問題があるし、3.3Vで動かすほうが楽なので、3.3Vで動かす予定
- サンプリング速度はデータシートを読む感じなんとなく間に合いそう。
- オフセット補正用の加算回路組むのは面倒だったので、4chモデルとかでdifferencial modeで読めば楽だったが、データシートを見る限り負の電圧は-0.6Vまでしか入力できないので扱えなさそうだった。

---

# ADCの選定3(問題点)
ADCのコンデンサに蓄えた電荷が空になるのに、2.7Vの場合700us、5Vの場合1.5msかかる。
つまり、**精度に問題がある可能性**
![w:450 center](./assets/adc-time.png)

---

# サンプルホールド回路
OFDMはタイミングがシビアなので、実部と虚部でADCのサンプリングがずれないようにするために使う。
